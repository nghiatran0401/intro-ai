...
VP -> V | V NP | V PP | Adv VP | VP Adv | V Adv | VP Conj VP
PP -> P NP
...
def np_chunk(tree):
    ...
    def has_np_subtree(t):
        """Check if tree t contains any NP subtrees (excluding itself)"""
        for subtree in t.subtrees():
            if subtree != t and subtree.label() == "NP":
                return True
        return False
    
    # Traverse all subtrees and find NP chunks
    for subtree in tree.subtrees():
        if subtree.label() == "NP" and not has_np_subtree(subtree):
